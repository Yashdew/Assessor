...
from api.resume_parser.parser import ResumeDataExtractor
...
@main.route('/api/v1/postResumeExp', methods=['POST'])
@cross_origin()
def index():
    dataList = list()
    if request.method == 'POST':
        try:
            ...
            for file in request.files.getlist('file'):
                ...
                if fileName.endswith('.pdf'):
                    file.save(file.filename)
                    ext = ResumeDataExtractor(fileName)
                    dataList.append(ext.parse_resume())
                    os.remove(fileName)
            return json.dumps(dataList)
        except Exception as e:
            return error(str(e.args), 415)
